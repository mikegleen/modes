...
def one_object(objelt, idnum, exhibition: Exhibition):
    ...
    # Insert the Exhibition elements so that they are in ascending date order
    for _edate, exhib in sorted(exhibs_to_insert, reverse=True):
...
def get_csv_dict(csvfile):
    ...
    :param csvfile: contains the accession number and optionally the exhibition
                    number in columns specified by --col_acc and --col_ex
    ...
    with codecs.open(csvfile, 'r', 'utf-8-sig') as mapfile:
        ...
        for n in range(_args.skiprows):
            next(reader)
        for row in reader:
            accnum = normalize_id(row[_args.col_acc])
            if _args.exhibition:
...
def main():
    ...
    exmap = get_csv_dict(_args.mapfile)  # col_acc -> exhibition #
